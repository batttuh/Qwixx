...
class _CreateGameViewState extends State<CreateGameView> {
  ...
  final String headline = localizor.tr(LocaleKeys.gameTime);
  final String subheadline = localizor.tr(LocaleKeys.createGame);
  @override
  void initState() {
            ...
            children: [
              ...
              Padding(
                  ...
                  child: SizedBox(
                      ...
                      child: Lottie.asset("assets/animation/createGame.json"))),
            ],
          ),
        ),
      ]),
    ...
    return ElevatedButton(
      onPressed: () async {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => const GameChoose(online: true)));
      },
      child: Text(localizor.tr(LocaleKeys.create)),
    );
  }
}
    ...
    return ElevatedButton(
      onPressed: () async {
        if (textController.text != "") {
          Future<User> response = client
              .join(DefaultUserModel().userModel(textController.text, null));
          response.then((value) => Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) =>
                      WaitingUser(side: value.room.sixSide, user: value))));
        }
      },
      child: Text(localizor.tr(LocaleKeys.join)),
    );
  }
}
